...
def train_test_split(X, y):
    X_train, X_test, y_train, y_test = model_sel.train_test_split(
...
def train_model(X_train, y_train):
    ...
    model = xgb.XGBClassifier(
        **params, enable_categorical=True, verbosity=1, device="cuda"
    )
...
def compare_current_to_champion(model, output_metrics, comparison_metric_type):
    ...
    if read:
        ...
        if (
            float(comparison_metric_champion)
            - float(output_metrics[comparison_metric_type])
            > -0.0001
        ):
            print("Champion outperforming new version, retaining champion")
            champion_model = joblib.load(
                f"{model_directory}/{model_hash_champion}.joblib"
            )
            ...
            output_json["confusion_matrix"] = str(
                output_metrics["confusion_matrix"].tolist()
            )  # tolist() ensures serializability
    else:
