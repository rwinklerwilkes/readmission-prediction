...
def run_calibration_process(uncalibrated_model, X_train, y_train, X_test, y_test):
    initial_calibration_error, initial_brier = calculate_calibration_errors(
        uncalibrated_model, X_test, y_test
    )
    print(
        f"Initial calibration error (perfectly calibrated = 0): {initial_calibration_error:0.3f}"
    )
    print(f"Initial Brier score (lower = better): {initial_brier}")

    calibrated_model = CalibratedClassifierCV(
        uncalibrated_model, method="isotonic", cv=5
    )
    ...
    final_calibration_error, final_brier = calculate_calibration_errors(
    ...
    print(
        f"Final calibration error (perfectly calibrated = 0): {final_metrics['final_calibration_error']:0.3f}"
    )
    print(
        f"Final Brier score (lower = better): {final_metrics['brier_calibrated']:0.3f}"
    )
