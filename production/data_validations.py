    ...
    schema = pa.DataFrameSchema(
        {
            ...
            "race": pa.Column(str),
            "gender": pa.Column(
                str, pa.Check(lambda s: s.isin(["Female", "Male", "Unknown/Invalid"]))
            ),
            "age": pa.Column(str),
            "weight": pa.Column(str),
            "admission_type_id": pa.Column(
                int, [pa.Check.greater_than(0), pa.Check.le(8)]
            ),  # Defined in data/IDS_mapping.csv
            "discharge_disposition_id": pa.Column(
                int, [pa.Check.greater_than(0), pa.Check.le(29)]
            ),  # Defined in data/IDS_mapping.csv
            "admission_source_id": pa.Column(
                int, [pa.Check.greater_than(0), pa.Check.le(26)]
            ),  # Defined in data/IDS_mapping.csv
            "time_in_hospital": pa.Column(int, pa.Check.greater_than(0)),
            "readmitted": pa.Column(
                str, pa.Check(lambda s: s.isin(["<30", ">30", "NO"]))
            ),
        }
    )
    return schema
...
def population_stability_index(old_df, new_df, grouping, pregrouped=False):
    if not pregrouped:
        ...
        old_ratio = old_df
        new_ratio = new_df
    compare_ratio = pd.merge(
        old_ratio, new_ratio, on=grouping, how="outer", suffixes=("_old", "_new")
    )
    compare_ratio = compare_ratio.fillna(0.0001)
    compare_ratio["diff"] = compare_ratio["ratio_old"] - compare_ratio["ratio_new"]
    compare_ratio["log_diff"] = np.log(
        compare_ratio["ratio_old"] / compare_ratio["ratio_new"]
    )
