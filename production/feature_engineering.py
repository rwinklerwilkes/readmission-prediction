...
def count_all_diag(df):
    diag = (
        pd.concat(
            [
                df["diag_1"].drop_duplicates(),
                df["diag_2"].drop_duplicates(),
                df["diag_3"].drop_duplicates(),
            ]
        )
        .drop_duplicates()
        .reset_index()
...
def join_diag_to_icd9(diag, icd9):
    ...
    diag_copy["diagnosis"] = diag_copy["diag"].str.replace(
...
def stash_data(df, diag):
    ...
    try:
        diag.iloc[:100].loc[:, ["diag", "total_ct"]].to_csv(
            f"{frontend_directory}/valid_diag.csv", index=False
        )
        df["race"].drop_duplicates().to_csv(
            f"{frontend_directory}/valid_race.csv", index=False
        )
        df["age"].drop_duplicates().to_csv(
            f"{frontend_directory}/valid_age.csv", index=False
        )
        df["gender"].drop_duplicates().to_csv(
            f"{frontend_directory}/valid_gender.csv", index=False
        )
        return True
...
def calculate_readmitted_rate(df, grouping=None):
    if grouping:
        rate = (
            df.groupby(grouping)
            .agg({"patient_nbr": "count", "readmitted_dummy": "sum"})
            .reset_index()
        )
...
def add_prior_admissions(df):
    df_prior = df.copy()
    df_prior["prior_readmissions"] = (
        df_prior.sort_values(["patient_nbr", "encounter_id"])
        .groupby("patient_nbr")
        .cumcount()
    )
    return df_prior
...
def icd9_featurization(icd9):
    ...
    icd9["long_tokenized"] = (
        icd9["long"].str.replace("[^a-zA-Z0-9\s]", "", regex=True).str.split(" ")
    )

    representative_term_first = (
        icd9.sort_values(["first_three", "diagnosis"])
        .groupby("first_three")
        .first()["long"]
        .reset_index()
    )
